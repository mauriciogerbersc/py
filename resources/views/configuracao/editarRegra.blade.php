...
<div class="content content-fixed">
    <div class="container">
        ...
        <h4 id="section4" class="mg-b-10">Editar Regra</h4>

        <div class="tx-14 mg-b-25">
            <form method="POST" action="/configuracao/regras/editar/{{$regra->id}}">
                @csrf
                <div class="form-row">
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="custom-select" name="categoria">
                                <option value="0" {{ $regra->categoria == 0 ? "selected" : ""}}>Full</option>
                                <option value="1" {{ $regra->categoria == 1 ? "selected" : ""}}>Basic</option>
                                <option value="2" {{ $regra->categoria == 2 ? "selected" : ""}}>Ambos</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="custom-select" name="variavel_id">
                            @foreach($variaveis as $variavel)
                                <option value="{{$variavel->id}}" {{ $variavel->id == $regra->variavel_id ? "selected" : ""}}>{{$variavel->nome}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="custom-select" name="pergunta_id">
                            @foreach($perguntas as $pergunta)
                                <option value="{{$pergunta->id}}"  {{ $pergunta->id == $regra->pergunta_id ? "selected" : ""}}>{{$pergunta->pergunta}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <textarea name="regra_negocio" rows="5" class="form-control">{{$regra->regra_de_negocio}}</textarea>
                    </div>
